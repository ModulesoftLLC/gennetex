...
import {
  ...
  Search, ShieldCheck, Users, Wifi, WifiOff, X, MapPinned, Smartphone, Sun, Moon, RefreshCw, Car,
} from 'lucide-react';
...
const baseNav = [
  ...
  ['inventory', 'Агуулах', Boxes], ['vehicles', 'Машины мэдээлэл', Car], ['locations', 'Realtime байршил', MapPinned],
...
export default function App() {
  ...
  const [devices,setDevices]=useState<ErpRecord[]>([]);const[vehicles,setVehicles]=useState<ErpRecord[]>([]);const[refreshKey,setRefreshKey]=useState(0);const[theme,setTheme]=useState<'dark'|'light'>(()=>(localStorage.getItem('gennetex-theme') as 'dark'|'light')||'dark');
  ...
  useEffect(() => { if(!profile)return; const off=[subscribeProfiles(setProfiles),subscribeCollection('attendance',setAttendance),subscribeCollection('service_calls',setCalls),subscribeCollection('inventory',setInventory),subscribeCollection('vehicles',setVehicles),subscribeCollection('device_approvals',setDevices)]; return()=>off.forEach(fn=>fn()); }, [profile,refreshKey]);
  ...
  const nav=[...baseNav.slice(0,9),...(canSeeRevenue?[['revenue','Миний орлого',DollarSign] as const]:[]),...baseNav.slice(9)];
